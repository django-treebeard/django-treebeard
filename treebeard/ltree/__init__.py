...
from django.db.models import F, OuterRef, Q, Subquery, Transform, Value
...
def generate_path(
    ...
    """
    Generate a new path value that will order the label before `before` and after `after`.
    """
    length = 1

    # We use digits and ascii letters to compose the alphabet for paths.
    # If no `before` is provided, then choose only from letters: this allows
    # room for digits to be used in future if nodes are inserted in front of this one
    # without having to move large chunks of the tree.

    char_choices = string.digits + string.ascii_letters if before else string.ascii_letters

    label_iter = itertools.product(char_choices, repeat=length)
    if prefix is None:
        prefix = []
    ...
    while length < 6:
        for label in label_iter:
            label = "".join(label)
            # Check if label meets constraints. If it does, use it
            if (not before or label < before) and (not after or (label > after)):
                value = PathValue(prefix + [label])
                if skip and value in skip:
                    continue
                return value
        length += 1

    raise ValueError("Failed to generate new path within constraints")
...
class LT_ComplexAddMoveHandler:
    def _get_new_path(self, prefix, insert_before, insert_after, nodes_moved=False, num_iters=0):
        ...
        siblings = None
        if insert_before:
            siblings = insert_before.get_siblings()
        elif insert_after:
            siblings = insert_after.get_siblings()
        ...
        try:
            path = generate_path(
                ...
                skip=siblings.values_list("path", flat=True) if siblings else None,
            )
        except ValueError:
            ...
            # We should never end up here - if we do, there is a bug
            if num_iters > 3:
                raise Exception("Something is broke")
...
class LT_Node(Node):
    ...
    node_order_by = []
    path = PathField(max_length=255, unique=True)
